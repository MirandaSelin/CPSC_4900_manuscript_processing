...
class GlyphEmbedder:
    ...
    def pad_to_square(self, glyph_image):
        """
        Pad glyph to square while preserving aspect ratio, then scale to fill most of the target size.
        
        Instead of keeping the glyph at its original tiny size, this scales it up to fill
        approximately 80% of the target dimension, making the features more visible to ResNet50.
        ...
        # First, make it square by padding
        max_dim = max(h, w)
        ...
        # Scale the glyph to fill ~80% of the target size (about 180 pixels)
        # This leaves a 22-pixel border on each side
        fill_ratio = 0.8
        scaled_size = int(self.target_size * fill_ratio)
        scaled_img = cv2.resize(square_img, (scaled_size, scaled_size))
        
        # Place scaled glyph in center of target-sized white canvas
        final_img = np.full((self.target_size, self.target_size), 255, dtype=glyph_image.dtype)
        border = (self.target_size - scaled_size) // 2
        final_img[border:border + scaled_size, border:border + scaled_size] = scaled_img
...
def main():
    """Process glyphs and generate embeddings"""
    
    parser = argparse.ArgumentParser(
        description='Generate ResNet50 embeddings for extracted glyphs',
...
Examples:
  ...
  python glyph_embedder.py /path/to/my_manuscript_config.ini
    ...
    try:
        ...
        # Create embedder with configured parameters
        embedder = GlyphEmbedder(target_size=embeddings_params['target_size'])
